...
By contributing to Geometry Dash, you agree with the [Developer Certificate of Origin (DCO)][DCO].
...
Use American English to keep consistency across Geometry Dash.
...
It allows to separate the actual name of the variable and the namespace, `geometryDash`.

Fields marked as `@Unique` must be prefixed with `geometryDash$` or `GEOMETRYDASH$` if the field is static and final.
Methods marked with `@Unique` do not need prefixes.


## Licensing & DCO

Geometry Dash is licensed under [LGPL 3.0][LICENSE], and it has a [Developer Certificate of Origin (DCO)][DCO], which you are required to agree with to contribute.
...
[LICENSE]: ./LICENSE "Geometry Dash license file"
